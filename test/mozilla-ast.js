// Testing Emojify <-> SpiderMonkey AST conversion
...
// Normalizes input AST for Emojify in order to get correct comparison.
    ...
    return estraverse.replace(ast, {
        enter: function(node, parent) {
            switch (node.type) {
                ...
                // Since Emojify doesn"t recognize different types of property keys,
                // decision on SpiderMonkey node type is based on check whether key
                // can be valid identifier or not - so we do in input AST.
                case "Property":
                    var key = node.key;
                    if (key.type === "Literal" && typeof key.value === "string" && Emojify.is_identifier(key.value)) {
                        node.key = {
                            type: "Identifier",
                            name: key.value
                        };
                    } else if (key.type === "Identifier" && !Emojify.is_identifier(key.name)) {
                        node.key = {
                            type: "Literal",
                            value: key.name
                        };
                ...
                // Emojify internally flattens all the expression sequences - either
...
module.exports = function(options) {
    console.log("--- Emojify <-> Mozilla AST conversion");
